...
class Task(models.Model):
    ...
    status = models.CharField(max_length=16, choices=StatusChoices.choices, default=StatusChoices.OPEN, null=False, blank=False)
    ...
    def clean(self):
        ...
        if self.due_date and self.due_date < now().date():
            raise ValidationError("The due date must be greater than or equal to the current date.")
