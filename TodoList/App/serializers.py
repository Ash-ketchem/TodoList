...
class TaskSerializer(serializers.ModelSerializer):
    tags = serializers.ListField(child=serializers.CharField(max_length=30), write_only=True, default=[])
    ...
    def validate(self, attrs):
        ...
        if due_date  and due_date < now().date() and status != Task.StatusChoices.COMPLETED:
    ...
    def update(self, instance, validated_data):
        ...
        if validated_data.get('status', '') ==  Task.StatusChoices.OVERDUE:
            ...
            "status": "Cannot update task to OVERDUE status."
        })
